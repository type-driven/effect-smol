...
const fetch: HttpClient.HttpClient = HttpClient.make((request, url, signal, fiber) => {
  ...
  let headers = options.headers
    // Deno types `HeadersInit` more strictly than lib.dom; cast to satisfy `deno check`
    ? Headers.merge(Headers.fromInput(options.headers as ReadonlyArray<[string, string]>), request.headers)
    : request.headers
